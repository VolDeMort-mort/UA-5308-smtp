...
{
    EXPECT_TRUE(m_dal->insert(make(1, "Inbox")));
    EXPECT_TRUE(m_dal->insert(make(2, "Inbox")));
}
...
{
    m_dal->insert(make(1, "Inbox"));
    m_dal->insert(make(1, "Sent"));
    m_dal->insert(make(2, "Inbox"));

    auto r = m_dal->findByUser(1);
    EXPECT_EQ(r.size(), 2u);
    for (const auto& f : r) EXPECT_EQ(f.user_id, 1);
}
...
{
    m_dal->insert(make(1, "Work"));
    auto f = m_dal->findByName(1, "Work");
...
{
    auto f1 = make(1, "Keep");   m_dal->insert(f1);
    auto f2 = make(1, "Delete"); m_dal->insert(f2);
...
{
    auto f = make(1, "Temp"); m_dal->insert(f);

    MessageDAL msgDal(m_db);
    Message msg; msg.user_id = 1; msg.folder_id = f.id.value();
    msg.receiver = "b@b.com"; msg.subject = "T"; msg.status = MessageStatus::Draft;
    msgDal.insert(msg);
