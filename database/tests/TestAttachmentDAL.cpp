...
protected:
    ...
    Attachment make(int64_t msg_id = 0,
                    const std::string& name = "file.pdf",
                    const std::string& path = "/files/file.pdf",
                    std::optional<std::string> mime = "application/pdf",
...
{
    m_dal->insert(make(m_msg1, "a.pdf", "/a.pdf"));
    m_dal->insert(make(m_msg1, "b.pdf", "/b.pdf"));
    m_dal->insert(make(m_msg2, "c.pdf", "/c.pdf"));
...
{
    auto a = make(); m_dal->insert(a);
    a.file_name = "renamed.pdf"; a.file_size = 9999;
...
{
    auto a = make(); m_dal->insert(a);
    a.mime_type = std::nullopt; a.file_size = std::nullopt;
    m_dal->update(a);
...
TEST_F(AttachmentDALTest, HardDelete_OnlyRemovesTarget)
{
    ...
    m_dal->insert(a1); m_dal->insert(a2);
