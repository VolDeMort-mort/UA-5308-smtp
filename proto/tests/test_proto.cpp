...
static std::thread run_io_context(boost::asio::io_context& ioc) {
...
template<typename T>
static T wait_future(std::future<T>& f, std::chrono::milliseconds timeout = 2000ms) {
...
struct SmtpTestSockets
{
	...
	std::shared_ptr<SMTPServerSocket> serverSocket;
	std::shared_ptr<SMTPClientSocket> clientSocket;
	...
	SmtpTestSockets() : work(boost::asio::make_work_guard(ioc)), acceptor(ioc, tcp::endpoint(tcp::v4(), 2525))
	{
		...
		clientSocket = std::make_shared<SMTPClientSocket>(ioc);

		auto server_ready_p = std::make_shared<std::promise<void>>();
		auto server_ready_f = server_ready_p->get_future();

		acceptor.async_accept([this, server_ready_p](const boost::system::error_code& ec, tcp::socket sock){
			if (ec){
				server_ready_p->set_exception(std::make_exception_ptr(std::runtime_error("accept failed: " + ec.message())));
				return;
			}
			serverSocket = std::make_shared<SMTPServerSocket>(std::move(sock));
		...
		clientSocket->connect(endpoint,[client_connected_p](const boost::system::error_code& ec){
			if (ec) 
				client_connected_p->set_exception(std::make_exception_ptr(std::runtime_error("connect failed: " + ec.message())));
			else
	...
	void start_tls_on_both(boost::asio::ssl::context& server_ctx, boost::asio::ssl::context& client_ctx){
		...
		serverSocket->start_tls(server_ctx,
			[server_p](const boost::system::error_code& ec){
				if (ec)server_p->set_exception(std::make_exception_ptr(std::runtime_error("server tls failed: " + ec.message())));
				else server_p->set_value();
			});

		clientSocket->start_tls(client_ctx,
			[client_p](const boost::system::error_code& ec){
				if (ec) client_p->set_exception(std::make_exception_ptr(std::runtime_error("client tls failed: " + ec.message())));
				else client_p->set_value();
			});

		wait_future(server_f);
		wait_future(client_f);
	}
...
TEST(SMTP_Socket_Tests, ClientToServer_AllCommandsParsed) {
	...
	for (const auto& cmd : commands) {
		...
		if (cmd.command == SEND_DATA) {
			sockets.serverSocket->readDataBlock(10000ms, [parsed_p](ClientCommand parsed, const boost::system::error_code& ec) {
				if (ec) parsed_p->set_exception(std::make_exception_ptr(std::runtime_error("server readCommand ec: " + ec.message())));
				else parsed_p->set_value(std::move(parsed));
				});
		}
		else {
			sockets.serverSocket->readCommand(10000ms, [parsed_p](ClientCommand parsed, const boost::system::error_code& ec) {
				if (ec) parsed_p->set_exception(std::make_exception_ptr(std::runtime_error("server readCommand ec: " + ec.message())));
				else parsed_p->set_value(std::move(parsed));
				});
		}
		...
		sockets.clientSocket->sendCommand(cmd, [write_p](const boost::system::error_code& ec) {
			if (ec) write_p->set_exception(std::make_exception_ptr(std::runtime_error("client sendCommand ec: " + ec.message())));
			else write_p->set_value();
			});

		wait_future(write_f);
		ClientCommand parsed = wait_future(parsed_f);
...
TEST(SMTP_Socket_Tests, ServerToClient_AllResponsesParsed) {
	...
	for (const auto& resp : responses) {
		...
		sockets.clientSocket->readResponse(2000ms, [parsed_p](ServerResponse resp, const boost::system::error_code& ec) {
			if (ec) parsed_p->set_exception(std::make_exception_ptr(std::runtime_error("client readResponse ec: " + ec.message())));
			else parsed_p->set_value(std::move(resp));
			});

		auto write_p = std::make_shared<std::promise<void>>();
		auto write_f = write_p->get_future();

		sockets.serverSocket->sendResponse(resp, [write_p](const boost::system::error_code& ec) {
			if (ec) write_p->set_exception(std::make_exception_ptr(std::runtime_error("server sendResponse ec: " + ec.message())));
			else write_p->set_value();
			});

		wait_future(write_f);
		ServerResponse parsed = wait_future(parsed_f);
