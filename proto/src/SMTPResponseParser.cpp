...
void SMTPResponseParser::ParseLine(const std::vector<std::string>& lines, ServerResponse& resp)
{
	...
	if (first.size() >= 3 && isdigit(static_cast<unsigned char>(first[0])) &&
		isdigit(static_cast<unsigned char>(first[1])) && isdigit(static_cast<unsigned char>(first[2])))
	{
...
bool SMTPResponseParser::TryParseCode(const std::string& line, int& out_code)
{
...
bool SMTPResponseParser::IsFinalSep(const std::string& line)
{
	if (line.size() >= 4)
	{
		char sep = line[3];
