	...
	m_acceptor.async_accept(
		[this](const boost::system::error_code& ec, tcp::socket socket) mutable
		{
			if (!ec)
			{
				try
				{
					auto server_socket = std::make_shared<SMTPServerSocket>(std::move(socket));
					auto session = std::make_shared<SMTPServerSession>(server_socket, m_has_tls ? m_ssl_ctx : nullptr);
