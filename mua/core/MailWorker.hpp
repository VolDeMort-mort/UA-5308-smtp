...
private:
	...
	bool ensureMailboxSelected(const std::string& folderName, std::string& error);
	bool ensureInboxSelected(std::string& error);
	std::vector<std::string> fetchFlagsBySeq(int64_t mailId);
	bool fetchMessagePayloadBySeq(int64_t mailId, std::string& rawEmail);
