...
std::string ExtractRfc822Payload(const std::vector<std::string>& lines)
{
    ...
    for (const auto& line : lines)
    {
        if (!started)
        {
            if (line.find("RFC822 {") != std::string::npos)
            {
                started = true;
            }
            continue;
...
void MailWorker::handleFetchMails(const FetchMailsCommand& cmd)
{
    ...
    {
        auto [selectOk, _] = imapExchange("SELECT " + QuoteImap(cmd.folderName));
        if (!selectOk)
        {
    ...
    {
        auto [ok, lines] = imapExchange("FETCH " + std::to_string(seqId) + " (FLAGS RFC822)");
        if (!ok)
        {
            continue;
        }

        const std::string rawEmail = ExtractRfc822Payload(lines);
        if (rawEmail.empty())
        {
            continue;
        }
...
void MailWorker::handleFetchMail(const FetchMailCommand& cmd)
{
    ...
    auto [ok, lines] = imapExchange("FETCH " + std::to_string(cmd.mailId) + " (RFC822)");
    if (!ok || lines.empty())
    {
...
void MailWorker::handleDownloadAttachment(const DownloadAttachmentCommand& cmd)
{
    ...
    {
        auto [selectOk, _0] = imapExchange("SELECT " + QuoteImap("INBOX"));
        if (!selectOk)
        {
            m_onResult(DownloadAttachmentResult{false, "SELECT INBOX failed", {}, {}, 0});
            return;
        }
        m_currentFolder = "INBOX";
    }

    auto [fetchOk, lines] = imapExchange("FETCH " + std::to_string(cmd.mailId) + " (RFC822)");
    if (!fetchOk || lines.empty())
    {
